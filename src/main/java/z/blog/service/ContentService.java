...
public class ContentService {
    ...
    public PageInfo<Article> getArticle(Article param, int pageNum, int pageSize) {
        ...
        pageSize = Optional.of(pageSize).filter(i -> (i > 0 && i < 9999)).orElse(20);
        //查询条件
        ...
        //分页数据
        List<Article> list = query.orderBy(T_ARTICLE.SORT.desc(), T_ARTICLE.CREATED.desc()).limit((pageNum - 1) * pageSize, pageSize).fetchInto(Article.class);
        //统计总数
    ...
    public PageInfo<Article> getArticle(MetaParam param) {
        ...
        //查询SQL
        SelectConditionStep<Record> query = select().from(T_RELATIONSHIP)
    ...
    public Article getArticle(Article param, String type) {
        ...
        if (PREV.equalsIgnoreCase(type)) {
            log.info("-> 上一篇文章");
            condition = T_ARTICLE.CREATED.gt(param.getCreated());
        } else if (NEXT.equalsIgnoreCase(type)) {
            log.info("-> 下一篇文章");
            condition = T_ARTICLE.CREATED.lt(param.getCreated());
        } else {
            return null;
        }
        //查询
        return select().from(T_ARTICLE)
                .where(
                        T_ARTICLE.AID.eq(
                                select(max(T_ARTICLE.AID)).from(T_ARTICLE).where(T_ARTICLE.TYPE.eq(POST)).and(condition)
                        )
